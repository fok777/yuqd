...
基于 Selenium + ICR 验证码识别的每日自动签到脚本，支持多账号、Cookie 缓存、代理、并发执行，可在 GitHub Actions 中定时运行。

## 功能特性

- ✅ 自动完成账户登录（Selenium 模拟浏览器）
- ✅ ICR 验证码识别（旋转分析 + 模板匹配）
...
| Secret | 说明 |
|--------|------|
| `RAINYUN_USER` | 用户名（多行支持多账号） |
| `RAINYUN_PASS` | 密码（多行需与用户名数量匹配） |
